...
if (isset($_POST['id_coach']) && isset($_POST['folder']) &&
    ...
    $result1 = $conexion->query("SELECT * FROM coach WHERE id = ".$id_coach);
...
';
        ...
        if(file_exists($estructura)){
            ...
            echo "<script> 
                alert('El nombre de la carpeta existe')
                </script>";

            echo "<h3>Capturadora Collageno</h3><br>";
            ...
            if(!$a){echo "ERROR al insertar el fichero";}   
             ...
             echo "<script> 
                alert('Se ha creado successfully')
                </script>";

            echo "<h3>Capturadora Web Estados Unidos</h3><br>";
            echo "<h3>Capturadora Web ha sido creada satisfactoriamente</h3>";
            echo "<a href=".$estructura." target='_blank' class='btn btn-success  '><span class='glyphicon glyphicon-thumbs-up'></span> Mostrar Web ''".$folder."''</a>";
            echo "<br/>";
            echo "<br/>";

            $insert = "INSERT INTO webcoach(coach,web,dir_name,country) VALUES('$id_coach','Collagen','$estructura','USA')";
              if ($conexion->query($insert) == TRUE) {
              }else{
...
<br>
          ...
          <label for="name">Entrenador :</label>
            ...
            <?php 
              ...
              echo $name_user;
              echo '<input type="hidden" value="'.$id_name.'" name="id_coach" readonle/>';
